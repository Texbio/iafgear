...
public class DefenseTrait extends SimpleTrait {
    ...
    private String protectionType; //unused currently
    ...
    public static final ITraitSerializer<DefenseTrait> SERIALIZER = new Serializer<>(
            ...
            DefenseTrait::new,
            (trait, json) -> {
                trait.protectionType = GsonHelper.getAsString(json, "protection_type", trait.getId().getPath());
            },
            (trait, buffer) -> {
                trait.protectionType = buffer.readUtf();
            },
            (trait, buffer) -> {
                buffer.writeUtf(trait.protectionType);
            }
    );

    /**
     * Currently damage prevention is done in {@link info.partonetrain.iafgear.event.ServerEvents#onEntityDamage(LivingHurtEvent)}
